...
export default async function CardDetailPage({
  ...
  const spendPercent =
    card.limitMinorUnits === 0
      ? 0
      : Math.min(100, (card.spendMinorUnits / card.limitMinorUnits) * 100)
  const widthClass =
    spendPercent === 0
      ? "w-0"
      : spendPercent <= 25
  ...
  return (
    <section className="p-4 sm:p-6">
      <Link href="/cards" className="text-sm text-blue-600 hover:underline">
        ← Back to cards
      </Link>
      <div className="mt-6 max-w-2xl">
        <div className="flex items-start justify-between">
          ...
          <span className="rounded-full bg-gray-100 px-3 py-1 text-sm capitalize dark:bg-gray-800">
            {card.status}
          </span>
        </div>
        <dl className="mt-8 grid gap-5 border-y border-gray-200 py-6 sm:grid-cols-2 dark:border-gray-800">
          ...
          <div>
            ...
            <dd className="mt-1">{formatDate(card.createdAt)}</dd>
          </div>
        </dl>
        <div className="mt-8">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-gray-500">
              {formatMoney(card.spendMinorUnits, card.currency)} of{" "}
              {formatMoney(card.limitMinorUnits, card.currency)}
            </span>
          </div>
          <div className="mt-2 h-3 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-800">
            <div
              className={`${spendPercent > 80 ? "bg-amber-500" : "bg-blue-500"} h-full ${widthClass}`}
            />
          </div>
          <p className="mt-2 text-sm text-gray-500">
            {spendPercent.toFixed(0)}% of limit used
          </p>
        </div>
        <div className="mt-8">
          ...
          <ul className="mt-2 space-y-1 text-sm text-gray-500">
            {card.statusHistory.map((event, index) => (
              <li key={`${event.changedAt}-${index}`} className="capitalize">
                {event.status} · {formatDate(event.changedAt)}
              </li>
            ))}
          </ul>
        </div>
